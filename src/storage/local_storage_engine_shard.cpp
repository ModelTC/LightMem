...
size_t LocalStorageEngine::pickWritableShard(const std::string &hash) const {
  ...
  // Prefer a stable mapping (hash -> shard). If that shard is not currently writable
  // (ownership/draining), DO NOT fall back in ring order: that can amplify load on the
  // next writable shard after long non-writable runs, causing severe write skew and
  // premature per-shard eviction when ownership is split across nodes.
  //
  // Instead, probe pseudo-random shard IDs derived from the hash until we find a writable shard.
  // This keeps writes approximately uniform over the writable set.
