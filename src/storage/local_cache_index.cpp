...
bool LocalCacheIndex::loadFromSnapshot(const std::string &filename) {
  ...
  for (uint64_t i = 0; i < count; i++) {
    ...
    // Insert into index (Hot-end insertion)
    // Since we saved Cold-to-Hot, reading sequentially and inserting at Head
    // will result in [Hot, ..., Cold] order in memory.
    // Wait:
    // Saved: Cold, ..., Hot
    // Read 1 (Cold): put_ready -> [Cold]
    // Read 2 (Medium): put_ready -> [Medium, Cold]
    // Read 3 (Hot): put_ready -> [Hot, Medium, Cold]
    // Correct.
